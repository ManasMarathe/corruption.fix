...
--
-- Rationale (see the schema comment above `vault` in src/db/schema.ts):
-- reporter contact info (email/phone) lives in the `vault` schema,
...
-- normally connects as cannot even read `vault.*` tables — schema-level
...
--              vault-touching code path. No privileges on the `vault`
--              schema at all (not even USAGE), so it cannot even see that
--              vault.* tables exist, let alone query them.
...
--              Has access to `vault.*` and, for FK-free correlation via
...
-- --- cf_app: public schema only, no vault access -----------------------
...
COMMENT ON ROLE cf_app IS
  ...
  'no grants at all on the vault schema — cannot read reporter_identities '
...
REVOKE ALL ON SCHEMA vault FROM cf_app;

-- --- cf_vault: vault schema only ----------------------------------------
...
COMMENT ON ROLE cf_vault IS
  ...
  'production). Full CRUD on vault.* only.';

GRANT CONNECT ON DATABASE current_database() TO cf_vault;
GRANT USAGE ON SCHEMA vault TO cf_vault;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA vault TO cf_vault;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA vault TO cf_vault;
