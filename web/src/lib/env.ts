...
const rawEnvSchema = z.object({
  ...
  // Groq key for the complaint chat assistant (/api/chat). The AI SDK's
  // Groq provider reads process.env.GROQ_API_KEY itself; this entry exists
  // for validation/documentation. Deliberately optional even in production:
  // env.ts is imported by every route, so making it required would take the
  // whole site down over a missing chat key. Unset degrades /api/chat alone.
